"""Ingest the order CSV into the Bronze Delta table."""

from pyspark.sql import DataFrame
from pyspark.sql.functions import current_timestamp

SOURCE_PATH = "/Volumes/ecommerce_sales/raw/source_files/orders.csv"
...
def validate_source(source_df: DataFrame) -> int:
    ...
    if source_df.columns != EXPECTED_COLUMNS:
        raise ValueError(
            "Order source columns do not match the expected CSV contract. "
    ...
    if source_count != EXPECTED_ROW_COUNT:
        raise ValueError(
            f"Order source must contain {EXPECTED_ROW_COUNT:,} rows; "
...
def ingest_orders() -> None:
    """Overwrite the Bronze order table with the current source snapshot."""
    ...
    print(
        f"Order ingestion complete: source_rows={source_count:,}, "
